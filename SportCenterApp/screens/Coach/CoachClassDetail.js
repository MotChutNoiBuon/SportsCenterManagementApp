...
const CoachClassDetail = () => {
    ...
    const loadClassDetails = async () => {
        try {
            ...
            if (response.data) {
                ...
                // Lấy số lượng học viên đã được duyệt
                ...
                // Lấy số lượng học viên đang chờ duyệt
                const pendingResponse = await api.get(`${API_ENDPOINTS.enrollments}?gym_class=${classId}&status=pending`);
    ...
    const handleStartClass = () => {
        navigation.navigate('ClassSession', { classId: classId });
    };
